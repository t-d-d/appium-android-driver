...
const REMOTE_INSTALL_TIMEOUT = 300000; // milliseconds
...
helpers.createADB = async function (javaVersion, udid, emPort, adbPort) {
  let adb = await ADB.createADB({javaVersion, adbPort});
...
helpers.reinstallRemoteApk = async function (adb, localApkPath, pkg,
  ...
  await retry(tries, async () => {
    ...
    try {
      await adb.installFromDevicePath(remotePath, {timeout: REMOTE_INSTALL_TIMEOUT});
    } catch (e) {
